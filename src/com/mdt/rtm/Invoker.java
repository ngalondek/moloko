      ...
      throws ServiceException
  {
    ...
    finally
    {
      if (connection != null && (response == null || connectionStrategy.keepAlive(response, context) == false))
      {
        ...
        {
          log.warn(new StringBuffer("Could not close properly the socket connection to '").append(connection.getRemoteAddress()).append("' on port ").append(
              connection.getRemotePort()), exception);
        }
      }
    }
