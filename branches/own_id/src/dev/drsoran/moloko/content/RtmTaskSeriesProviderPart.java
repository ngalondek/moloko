...
public class RtmTaskSeriesProviderPart extends AbstractRtmProviderPart
{
   ...
   { TaskSeries._ID, TaskSeries.RTM_ID, TaskSeries.TASKSERIES_CREATED_DATE,
    ...
    TaskSeries.LOCATION_ID, TaskSeries.RTM_LOCATION_ID, TaskSeries.LIST_ID,
    TaskSeries.RTM_LIST_ID, TaskSeries.TAGS };
   ...
   public final static ContentValues getContentValues( RtmTaskSeries taskSeries )
   {
      ...
      values.put( TaskSeries.RTM_ID, taskSeries.getRtmId() );
      values.put( TaskSeries.LIST_ID, taskSeries.getListId() );
      values.put( TaskSeries.RTM_LIST_ID, taskSeries.getRtmListId() );
      ...
      if ( taskSeries.getLocationId() != RtmEntity.NO_ID )
         values.put( TaskSeries.LOCATION_ID, taskSeries.getLocationId() );
      else
         values.putNull( TaskSeries.LOCATION_ID );
      
      if ( !TextUtils.isEmpty( taskSeries.getRtmLocationId() ) )
         values.put( TaskSeries.RTM_LOCATION_ID, taskSeries.getRtmLocationId() );
      else
         values.putNull( TaskSeries.RTM_LOCATION_ID );
   ...
   private final static RtmTaskSeries createRtmTaskSeries( ContentProviderClient client,
                                                           Cursor c )
   {
      final String taskSeriesId = c.getString( COL_INDICES.get( TaskSeries.RTM_ID ) );
      ...
      if ( ok )
      {
         ...
         return new RtmTaskSeries( c.getLong( COL_INDICES.get( TaskSeries._ID ) ),
                                   taskSeriesId,
                                   c.getLong( COL_INDICES.get( TaskSeries.LIST_ID ) ),
                                   c.getString( COL_INDICES.get( TaskSeries.RTM_LIST_ID ) ),
                                   ...
                                   Queries.getOptLong( c,
                                                       ...
                                                       RtmEntity.NO_ID ),
                                   Queries.getOptString( c,
                                                         COL_INDICES.get( TaskSeries.RTM_LOCATION_ID ) ),
                                   Queries.getOptString( c,
   ...
   public void create( SQLiteDatabase db ) throws SQLException
   {
      ...
      db.execSQL( "CREATE TABLE " + path + " ( " + TaskSeries._ID
         + " INTEGER NOT NULL AUTOINCREMENT, " + TaskSeries.RTM_ID
         ...
         + TaskSeries.LOCATION_ID + " INTEGER, " + TaskSeries.RTM_LOCATION_ID
         + " TEXT, " + TaskSeries.LIST_ID + " INTEGER NOT NULL, "
         + TaskSeries.RTM_LIST_ID + " TEXT NOT NULL, " + TaskSeries.TAGS
         + " TEXT, " + "CONSTRAINT PK_TASKSERIES PRIMARY KEY ( \""
      ...
      db.execSQL( "CREATE TRIGGER " + path
         ...
         + " WHERE " + Participants._ID + " = old." + TaskSeries._ID + "; END;" );
      
      // Trigger: If RTM_LIST_ID gets updated, we update LIST_ID also.
      db.execSQL( "CREATE TRIGGER " + path + "_update_list_id AFTER UPDATE OF "
         + TaskSeries.RTM_LIST_ID + " ON " + path + " BEGIN UPDATE " + path
         + " SET " + TaskSeries.LIST_ID + " = (SELECT " + Lists._ID + " FROM "
         + Lists.PATH + " WHERE " + Lists.RTM_ID + " = new."
         + TaskSeries.RTM_LIST_ID + ")" + "; END;" );
      
      // Trigger: If RTM_LOCATION_ID gets updated, we update LOCATION_ID also.
      db.execSQL( "CREATE TRIGGER " + path
         + "_update_location_id AFTER UPDATE OF " + TaskSeries.RTM_LOCATION_ID
         + " ON " + path + " BEGIN UPDATE " + path + " SET "
         + TaskSeries.LOCATION_ID + " = (SELECT " + Locations._ID + " FROM "
         + Locations.PATH + " WHERE " + Locations.RTM_ID + " = new."
         + TaskSeries.RTM_LOCATION_ID + ")" + "; END;" );
   }
