...
import dev.drsoran.rtm.sync.SyncException;
import dev.drsoran.rtm.sync.SyncTime;


public class RtmSyncService implements IRtmSyncService
{
   ...
   public RtmSyncService( IRtmSyncPartner syncPartner,
      IRtmContentRepository contentRepository,
   ...
   @Override
   public void performIncomingSync( SyncTime syncTime ) throws RtmServiceException
   {
      ...
      catch ( SyncException e )
      {
         ...
         {
            throw new RtmServiceException( e.getCause() );
         }
      }
   }
   ...
   @Override
   public void performOutgoingSync( SyncTime syncTime ) throws RtmServiceException
   {
      checkSyncPartner( syncPartner );
      checkSyncTime( syncTime );
   ...
   @Override
   public void performFullSync( SyncTime syncTime ) throws RtmServiceException
   {
      checkSyncPartner( syncPartner );
      checkSyncTime( syncTime );
   ...
   private void syncIncoming( IRtmSyncPartner syncPartner, SyncTime syncTime ) throws SyncException
   {
      try
      {
         ...
         performIncomingRtmSettingsSync( syncPartner, syncTime );
      }
      catch ( Exception e )
      {
   ...
   private void performIncomingRtmSettingsSync( IRtmSyncPartner syncPartner,
                                                SyncTime syncTime ) throws SyncException
   {
      final IRtmSyncHandler syncHandler = syncHandlerFactory.createRtmSettingsSyncHandler();
      performIncomingSyncImpl( syncHandler, syncTime );
   }
