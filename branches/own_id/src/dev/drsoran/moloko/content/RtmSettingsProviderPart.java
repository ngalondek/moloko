...
public class RtmSettingsProviderPart extends AbstractRtmProviderPart
{
   ...
   { Settings._ID, Settings.SYNC_TIMESTAMP, Settings.TIMEZONE,
    ...
    Settings.RTM_DEFAULTLIST_ID, Settings.LANGUAGE };
   ...
   public final static ContentValues getContentValues( RtmSettings settings )
   {
      ...
      if ( settings != null )
      {
         ...
         if ( settings.getDefaultListId() != dev.drsoran.moloko.Settings.NO_DEFAULT_LIST_ID )
            values.put( Settings.DEFAULTLIST_ID, settings.getDefaultListId() );
         else
            values.putNull( Settings.DEFAULTLIST_ID );
         
         if ( !TextUtils.isEmpty( settings.getRtmDefaultListId() ) )
            values.put( Settings.RTM_DEFAULTLIST_ID,
                        settings.getRtmDefaultListId() );
         else
            values.putNull( Settings.RTM_DEFAULTLIST_ID );
         ...
         if ( ok )
         {
            settings = new RtmSettings( Queries.getOptDate( c,
                                        ...
                                        Queries.getOptLong( c,
                                                            ...
                                                            dev.drsoran.moloko.Settings.NO_DEFAULT_LIST_ID ),
                                        Queries.getOptString( c,
                                                              COL_INDICES.get( Settings.RTM_DEFAULTLIST_ID ) ),
                                        Queries.getOptString( c,
   ...
   public void create( SQLiteDatabase db ) throws SQLException
   {
      db.execSQL( "CREATE TABLE "
         ...
         + Settings.DEFAULTLIST_ID + " INTEGER, " + Settings.RTM_DEFAULTLIST_ID
         + " TEXT, " + Settings.LANGUAGE
         + " TEXT, CONSTRAINT defaultlist FOREIGN KEY ( "
         + Settings.DEFAULTLIST_ID + ") REFERENCES " + Lists.PATH + "( "
         + Lists._ID + " ) );" );
      
      // Trigger: If RTM_DEFAULTLIST_ID gets updated, we update DEFAULTLIST_ID also.
      db.execSQL( "CREATE TRIGGER " + path
         + "_update_defaultlist_id AFTER UPDATE OF "
         + Settings.RTM_DEFAULTLIST_ID + " ON " + path + " BEGIN UPDATE "
         + path + " SET " + Settings.DEFAULTLIST_ID + " = (SELECT " + Lists._ID
         + " FROM " + Lists.PATH + " WHERE " + Lists.RTM_ID + " = new."
         + Settings.RTM_DEFAULTLIST_ID + ")" + "; END;" );
   }
