...
public class AddTaskActivity extends AbstractTaskEditActivity
{
   ...
   protected InitialValues onCreateImpl( Intent intent )
   {
      ...
      long listId = RtmEntity.NO_ID;
      
      if ( intent.hasExtra( Tasks.LIST_ID ) )
         listId = intent.getLongExtra( Tasks.LIST_ID, RtmEntity.NO_ID );
      ...
      else
      {
            ...
            && listId == RtmEntity.NO_ID; )
         {
            ...
            if ( listId != RtmEntity.NO_ID )
               i.remove();
         }
      }
      
      // Try to determine the location ID
      long locationId = RtmEntity.NO_ID;
      
      if ( intent.hasExtra( Tasks.LOCATION_ID ) )
         locationId = intent.getLongExtra( Tasks.LOCATION_ID, RtmEntity.NO_ID );
   ...
   private long getListIdByName( String listName )
   {
      if ( listNames_2_listId != null )
         return listNames_2_listId.get( listName.toLowerCase() );
      
      return RtmEntity.NO_ID;
   }
   ...
   private long getLocationIdByName( String locationName )
   {
      if ( locationNames_2_locationIds != null )
         return locationNames_2_locationIds.get( locationName.toLowerCase() );
