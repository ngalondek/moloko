...
public class NoteActivity extends Activity
{
   ...
   protected void onResume()
   {
      ...
      if ( client != null )
      {
         ...
         final long taskId = intent.getLongExtra( Notes.TASKSERIES_ID, -1 );
         
         if ( taskId == -1 )
         {
            found = getSingleNote( client,
                                   Long.parseLong( intent.getData()
                                                         .getLastPathSegment() ) );
         }
         else
         {
            found = getAllNotesOfTask( client,
                                       taskId,
                                       intent.getLongExtra( Notes._ID, -1 ) );
         }
   ...
   private boolean getSingleNote( ContentProviderClient client, long noteId )
   {
      boolean found = false;
      ...
      if ( rtmTaskNotes != null )
      {
         ...
         else
         {
            ...
            if ( startNoteId != -1 )
            {
               ...
               {
                  foundStartNote = notes.get( notePos ).getId() == startNoteId;
